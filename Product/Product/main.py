...
def search():
    ...
    # Generate Amazon and Flipkart search URLs
    #amazon_url = generate_amazon_url(product_name)
    #flipkart_url = generate_flipkart_url(product_name)

    # Fetch the product details (dummy implementation for now)
    amazon_data = get_min_price_amazon_product(search_term=product_name,filename='amazon_products.csv')

    flipkart_data=fetch_flipkart_min_price_product(product_name=product_name, target_model=product_name)

    return render_template("result.html", amazon=amazon_data, flipkart=flipkart_data)

# Function to scrape Flipkart data, save to CSV, and find the product with minimum price
def fetch_flipkart_min_price_product(product_name, target_model, filename='flipkart_products.csv'):
    HEADERS = {
    ...
    if response.status_code == 200:
        ...
        titles = soup.find_all('div', {'class': 'KzDlHZ'})  # Product titles
        prices = soup.find_all('div', {'class': 'Nx9bqj _4b5DiR'})  # Product prices
        ratings = soup.find_all('div', {'class': 'XQDdHH'})  # Product ratings
        ...
        for i in range(min(len(titles), len(prices), len(ratings))): # type: ignore
            ...
            if target_model.lower() in title.lower():
                ...
                product_url = 'https://www.flipkart.com' + product_container['href'] if product_container else 'URL not available'
                product = {
                    'title': title,
                    'price': float(prices[i].text.replace('₹', '').replace(',', '').strip()),
    ...
    if products:
        ...
        # Find and display product with minimum price
...
def get_min_price_amazon_product(search_term, filename='amazon_products.csv'):
    ...
    def is_relevant_product(title, search_term):
        ...
        # Check for exact inclusion of search term in title
        return search_term_lower in title_lower

        # Optional fuzzy matching (comment out if not needed)
        # return SequenceMatcher(None, search_term_lower, title_lower).ratio() > 0.7
    ...
    for product in products:
        ...
        if title and price:
            ...
            if is_relevant_product(title_text, search_term):  # Filter based on title
    ...
    # Find product with minimum price from the list of products
    ...
    if min_price_product:
        ...
        print(f"No valid product found for '{search_term}'")
